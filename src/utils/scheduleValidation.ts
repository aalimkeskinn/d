...
export const checkSlotConflict = (
  ...
  if (!(DAYS as any).includes(sanitizedDay) || !PERIODS.includes(sanitizedPeriod)) {
  ...
  if (mode === 'teacher') {
    ...
    const conflictingSchedules = allSchedules.filter(schedule => {
      ...
      const hasConflict = slot?.classId === targetId && slot.classId !== 'fixed-period' && slot.classId !== 'KULÜP' && !slot.classId.startsWith('kulup-virtual-class-');
    ...
    if (teacherSchedule) {
      ...
      // FIXED: Check if teacher is already assigned to a different class (not fixed period, not KULÜP)
      if (existingSlot?.classId &&
        existingSlot.classId !== currentEntityId &&
        existingSlot.classId !== 'fixed-period' &&
        existingSlot.classId !== 'KULÜP' &&
        !existingSlot.classId.startsWith('kulup-virtual-class-')) {
