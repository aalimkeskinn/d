...
const Teachers = () => {
  ...
  const { data: subjects, update: updateSubject } = useFirestore<Subject>('subjects');
  ...
  // YENİ: Ders dağıtım şekilleri için state
  const [distributionPatterns, setDistributionPatterns] = useState<Record<string, string>>({});
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingTeacher) {
        ...
        // YENİ: Dağıtım şekillerini güncelle
        for (const [subjectId, pattern] of Object.entries(distributionPatterns)) {
          const subject = subjects.find(s => s.id === subjectId);
          if (subject && subject.distributionPattern !== pattern) {
            await updateSubject(subjectId, { distributionPattern: pattern });
          }
        }
  ...
  const resetForm = () => { setFormData({ name: '', branch: '', levels: [], subjectIds: [] }); setDistributionPatterns({}); setEditingTeacher(null); setIsModalOpen(false); };
  const handleEdit = (teacher: Teacher) => {
    // Öğretmenin derslerinin mevcut dağıtım şekillerini al
    const patterns: Record<string, string> = {};
    classes.forEach(cls => {
      const assignment = cls.assignments?.find(a => a.teacherId === teacher.id);
      if (assignment) {
        assignment.subjectIds.forEach(subjectId => {
          const subject = subjects.find(s => s.id === subjectId);
          if (subject && subject.distributionPattern) {
            patterns[subjectId] = subject.distributionPattern;
          }
        });
      }
    });
    setDistributionPatterns(patterns);
    setFormData({ name: teacher.name, branch: teacher.branch, levels: teacher.levels || [teacher.level], subjectIds: teacher.subjectIds || [], });
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {sortedTeachers.map((teacher) => {
                  ...
                  let clubHours = 0; // KULÜP saatleri ayrı sayılacak
                  if (teacherSchedule) {
                    Object.values(teacherSchedule.schedule).forEach(day => {
                      Object.values(day).forEach(slot => {
                        // ÖNEMLİ: isFixed kontrolü kaldırıldı - sabit dersler de sayılmalı!
                        if (slot && slot.classId) {
                          // KULÜP sanal sınıflarını ayrı say
                          if (slot.classId.startsWith('kulup-virtual-class-') || slot.classId === 'KULÜP') {
                            clubHours++;
                          } else {
                            ...
                            {allLevelsForTeacher.map(level => {
                              ...
                              return (
                                <div key={level} className="flex items-center text-xs space-x-2">
                                  ...
                                  {/* KULÜP saatleri - sadece ilk seviye için göster */}
                                  {level === allLevelsForTeacher[0] && clubHours > 0 && (
                                    <div className="px-2 py-1 rounded-full inline-flex items-center gap-1 bg-purple-100 text-purple-800">
                                      + {clubHours} Kulüp
                                    </div>
                                  )}
                                </div>
      ...
      <Modal isOpen={isModalOpen} onClose={resetForm} title={editingTeacher ? 'Öğretmen Düzenle' : 'Yeni Öğretmen Ekle'} size="lg">
        <form onSubmit={handleSubmit}>
          ...
          <div className="mt-6 pt-6 border-t border-gray-200">
            <h3 className="text-lg font-medium text-gray-900 mb-4 flex items-center"><BookOpen className="w-5 h-5 mr-2 text-ide-primary-600" />Dersler</h3>
            <div className="space-y-2 p-3 border border-gray-300 rounded-lg bg-gray-50 max-h-60 overflow-y-auto">
              {filteredSubjectsForModal.map((subject) => (
                <label key={subject.id} className="flex items-center space-x-3 cursor-pointer p-2 hover:bg-gray-100 rounded-md">
                  <input type="checkbox" checked={formData.subjectIds.includes(subject.id)} onChange={() => handleSubjectToggle(subject.id)} className="w-4 h-4 text-ide-primary-600 bg-gray-100 border-gray-300 rounded focus:ring-ide-primary-500" />
            ...
            {formData.subjectIds.length > 0 && (<p className="text-xs text-ide-primary-600 mt-2">✨ {formData.subjectIds.length} ders seçildi.</p>)}
          </div>

          {/* YENİ: Ders Dağıtım Şekilleri Bölümü - Sadece Düzenleme Modunda */}
          {editingTeacher && Object.keys(distributionPatterns).length > 0 && (
            <div className="mt-6 pt-6 border-t border-gray-200">
              <h3 className="text-lg font-medium text-gray-900 mb-4 flex items-center">
                <Clock className="w-5 h-5 mr-2 text-orange-500" />
                Ders Dağıtım Şekilleri
              </h3>
              <p className="text-xs text-gray-500 mb-3">Örnek format: 2+2+2 veya 3+3+2+1</p>
              <div className="space-y-3 p-3 border border-gray-300 rounded-lg bg-orange-50 max-h-60 overflow-y-auto">
                {Object.entries(distributionPatterns).map(([subjectId, pattern]) => {
                  const subject = subjects.find(s => s.id === subjectId);
                  if (!subject) return null;
                  return (
                    <div key={subjectId} className="flex items-center justify-between gap-3 bg-white p-2 rounded-md border border-orange-100">
                      <div className="flex-1 min-w-0">
                        <span className="text-sm font-medium text-gray-800 block truncate">{subject.name}</span>
                        <span className="text-xs text-gray-500">{subject.className || 'Genel'} - {subject.weeklyHours} saat</span>
                      </div>
                      <input
                        type="text"
                        value={pattern}
                        onChange={(e) => setDistributionPatterns(prev => ({ ...prev, [subjectId]: e.target.value }))}
                        placeholder="Örn: 2+2+1"
                        className="w-28 px-2 py-1.5 text-sm border border-orange-300 rounded-md focus:ring-2 focus:ring-orange-500 focus:border-orange-500"
                      />
                    </div>
                  );
                })}
