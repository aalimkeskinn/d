  ...
  const features = [
    {
      ...
      color: 'text-ide-primary-600',
      bgColor: 'bg-ide-primary-50',
      path: '/subjects'
    },
    {
      icon: Users,
    ...
    },
    {
      ...
      color: 'text-ide-secondary-600',
      bgColor: 'bg-ide-secondary-50',
      path: '/classes'
    },
    {
      icon: Zap,
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div className="text-center">
          ...
          <h1 className="text-xl md:text-2xl font-semibold text-gray-900 mb-2">
            ide okulları Ders Programı Sistemi
          </h1>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="bg-gradient-to-br from-ide-primary-50 via-white to-ide-secondary-50 rounded-2xl shadow-lg border border-ide-primary-100 p-8">
            <div className="flex items-center justify-between mb-6">
              <div>
                <h2 className="text-2xl font-semibold text-ide-primary-800 flex items-center tracking-tight">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {sortedTemplates.map((template) => (
                ...
                >
                  {/* Durum Rozeti */}
                  <span className={`absolute top-4 right-4 px-3 py-1 rounded-full text-xs font-semibold shadow-sm border tracking-wider
                  ...
                  >
                    {template.status === 'published' ? 'YAYINLANDI' : template.status === 'draft' ? 'TASLAK' : 'ARŞİVLENDİ'}
                  </span>
                  {/* Başlık ve Dönem */}
                  <div className="flex items-center space-x-3 mb-2">
                    <Calendar className="w-6 h-6 text-ide-primary-600 flex-shrink-0" />
                    <h3 className="font-semibold text-lg text-ide-primary-900 truncate group-hover:text-ide-primary-700 transition-colors">
                      {template.name}
                    </h3>
                  </div>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div className="bg-white rounded-lg p-4 shadow-sm border border-gray-100">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-center">
            ...
            <div>
              <div className="text-lg font-semibold text-ide-secondary-600">{classes.length}</div>
              <div className="text-xs text-gray-600">Sınıf</div>
            </div>
            <div>
              <div className="text-lg font-semibold text-ide-primary-600">{subjects.length}</div>
              <div className="text-xs text-gray-600">Ders</div>
            </div>
            <div>
              <div className="text-lg font-semibold text-ide-accent-500">{schedules.length}</div>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div className="bg-white rounded-lg p-4 shadow-sm border border-gray-100">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-center">
            <div>
              <div className="text-lg font-semibold text-gray-900">Sihirbaz Sistemi</div>
              <div className="text-xs text-gray-600">Adım adım otomatik oluşturma</div>
            </div>
            <div>
              <div className="text-lg font-semibold text-gray-900">Zaman Kısıtlamaları</div>
              <div className="text-xs text-gray-600">Esnek kısıtlama yönetimi</div>
            </div>
            <div>
              <div className="text-lg font-semibold text-gray-900">Çakışma Kontrolü</div>
              <div className="text-xs text-gray-600">Otomatik çakışma tespiti</div>
            </div>
            <div>
              <div className="text-lg font-semibold text-gray-900">PDF Çıktı</div>
