                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex flex-wrap gap-1">
                        {getSubjectLevelsDisplay(subject).map((level, i) => (
                          <span key={i} className={`px-2 py-1 text-xs rounded-full ${level === 'Anaokulu' ? 'bg-green-100 text-green-800' : level === 'İlkokul' ? 'bg-blue-100 text-blue-800' : 'bg-ide-primary-100 text-ide-accent-800'}`}>
                            {level}
                          </span>
                        ))}
                      </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {suggestions.map((suggestion, index) => (
                    <button
                      ...
                      className={`px-3 py-2 text-sm rounded-lg border transition-all duration-200 ${formData.distributionPattern === suggestion
