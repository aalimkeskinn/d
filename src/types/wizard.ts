...
/**
 * Sabit ders yerleştirme için kullanılan arayüz.
 * Belirli bir öğretmen-sınıf-ders kombinasyonunu belirli bir gün ve saate sabitler.
 */
...
export interface SubjectTeacherMapping {
  ...
  classId: string;
  subjectId: string;
  subjectName?: string; // YENİ: Ders adı (getSafeBlockKey için)
  ...
  distribution?: number[]; // YENİ: Dersin dağıtım şekli (örn: [2, 2, 2])
...
export interface EnhancedGenerationResult {
  ...
  warnings: string[];
  errors: string[];
  finalGrid: { [classId: string]: { [day: string]: { [period: string]: any } } };
}
