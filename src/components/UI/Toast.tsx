  ...
  return (
    <div
      ...
      style={{
        ...
        ...(type === 'error' && {
          animation: 'fadeInUp 0.5s cubic-bezier(0.16, 1, 0.3, 1) forwards, subtleGlow 3s ease-in-out infinite',
          boxShadow: '0 15px 30px -10px rgba(220, 38, 38, 0.3), 0 0 0 1px rgba(220, 38, 38, 0.1)'
        })
      }}
    >
      {/* Error toast special background */}
      {type === 'error' && (
        <div className="absolute inset-0 bg-gradient-to-r from-red-100 via-white to-red-100 opacity-40"></div>
      )}

      <div className="p-6 relative z-10">
        <div className="flex items-start">
          <div className="flex-shrink-0">
            <Icon className={`h-8 w-8 ${iconColors[type]} ${type === 'error' ? 'animate-bounce-subtle' : ''}`} />
          </div>
          <div className="ml-4 w-0 flex-1">
            <p className={`text-lg font-bold leading-tight ${type === 'error' ? 'text-ide-accent-900' : 'text-ide-gray-900'
              }`}>
              {title}
            </p>
            {message && (
              <p className={`mt-3 text-sm leading-relaxed whitespace-pre-line ${type === 'error' ? 'text-ide-accent-800 font-semibold' : 'text-ide-gray-700'
                }`}>
                {message}
              </p>
            )}
          ...
          <div className="ml-4 flex-shrink-0 flex">
            <button
              onClick={handleClose}
              className={`rounded-full p-2 inline-flex transition-all duration-200 hover:scale-110 focus:outline-none focus:ring-2 focus:ring-offset-2 ${type === 'error'
      ...
      {/* Error toast special effects omitted for elegance */}
    </div>
  );
};
