  ...
  return (
    <div style={{
            ...
            return (
              <tr key={period} style={{ backgroundColor: bgColor }}>
                ...
                {DAYS.map(day => {
                  ...
                  const teacher = (slot.teacher?.id === 'KULÜP' || slot.teacher?.id.startsWith('kulup-virtual-teacher-') || slot.teacher?.id.startsWith('generic-teacher-kulup')) ? { name: 'KULÜP' } as any : teachers.find(t => t.id === slot.teacher?.id);
      ...
      <div style={{ marginTop: '10mm', display: 'flex', justifyContent: 'space-between', fontSize: '10px', color: '#666666' }}>
        ...
        <div>ide okulları Ders Programı</div>
      </div>
    </div>
  );
};
