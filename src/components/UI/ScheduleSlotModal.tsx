...
const ScheduleSlotModal: React.FC<ScheduleSlotModalProps> = ({
  ...
  // Deduplicate Subjects for the Dropdown & Merge levels for filtering
  ...
  const sortedSubjects = Array.from(uniqueSubjectsMap.values()).sort((a, b) => a.name.localeCompare(b.name, 'tr'));
  const subjectOptions = sortedSubjects.map(subject => ({
    value: subject.id,
    label: subject.name
  }));

  // Filtering Classes Logic
  const selectedSubject = subjects.find(s => s.id === selectedSubjectId);

  // Find ALL subjects with the same name to merge their levels (robustness against duplicate/fragmented data)
  const normalizedSelectedSubjectName = selectedSubject?.name.toLocaleUpperCase('tr-TR').trim();
  const matchingSubjects = selectedSubject ? subjects.filter(s => s.name.toLocaleUpperCase('tr-TR').trim() === normalizedSelectedSubjectName) : [];

  // Collect all levels from all matching subjects
  const allSubjectLevels = new Set<string | number>();
  matchingSubjects.forEach(s => {
    const levels = (s.levels || (s.level ? [s.level] : [])).map(l => String(l).trim());
    levels.forEach(l => allSubjectLevels.add(l));
  });

  const subjectLevels = Array.from(allSubjectLevels);

  const filteredClasses = classes.filter(cls => {
    if (!selectedSubjectId || subjectLevels.length === 0) return true;
    const isMatch = subjectLevels.includes(cls.level as any) || subjectLevels.some(l => String(l) === String(cls.level));
    return isMatch;
  });
        ...
        <div className="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-md sm:w-full">
          <div className="bg-white px-4 pt-5 pb-4 sm:p-6">
            ...
            <div className="space-y-4">
              {/* Subject selection - only show if there are multiple subjects AND not in teacher mode (where subject is fixed) */}
              {subjects.length > 1 && mode !== 'teacher' && (
                <Select
                  label="Ders Seçin"
