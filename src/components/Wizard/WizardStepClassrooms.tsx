...
// import Select from '../UI/Select'; // Removed unused
...
const WizardStepClassrooms: React.FC<WizardStepClassroomsProps> = ({
    ...
    if (classroomsData.length > 0 && (!data.classrooms || data.classrooms.length === 0)) {
      onUpdate({
        classrooms: classroomsData.map((c: any) => ({
          id: c.id,
          name: c.name || 'İsimsiz Derslik',
          type: c.type || 'normal',
          capacity: c.capacity || 30,
          floor: c.floor || '1',
          building: c.building || 'Ana Bina',
          equipment: c.equipment || [],
          shortName: c.shortName || generateShortName(c.name || ''),
  ...
  const getTypeColor = (type: string) => {
    const colors = {
      ...
      laboratory: 'bg-ide-accent-100 text-ide-accent-800',
      workshop: 'bg-orange-100 text-orange-800',
      gym: 'bg-green-100 text-green-800',
      library: 'bg-ide-primary-100 text-ide-primary-800',
