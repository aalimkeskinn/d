...
const WizardStepTeachers: React.FC<WizardStepTeachersProps> = ({
  ...
  wizardData,
  all_classes,
  teachers: droppedTeachers,
}) => {
  const { data: firestoreTeachers, add: addTeacher, update: updateTeacher, remove: removeTeacher } = useFirestore<Teacher>('teachers');

  // Use passed teachers (CSV) or fallback to Firestore
  const teachers = droppedTeachers && droppedTeachers.length > 0 ? droppedTeachers : firestoreTeachers;
  ...
  const filteredTeachers = sortedTeachers.filter(t =>
    ...
    t.branch.toLocaleLowerCase('tr-TR').includes(searchTerm.toLocaleLowerCase('tr-TR'))
  );
        ...
        <div className="flex flex-1 max-w-md mx-4">
          <div className="relative w-full">
            <span className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
              <Plus className="h-4 w-4 text-gray-400 rotate-45" /> {/* Using Plus rotated as a search icon fallback if Search not imported */}
            </span>
            <input
              type="text"
            ...
            return (
              <div key={teacher.id} className={`relative bg-white rounded-lg border-2 p-4 transition-all cursor-pointer ${isSelected ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:border-gray-300'}`} onClick={() => handleTeacherToggle(teacher.id)}>
                ...
                <div className="pr-8">
                  ...
                  <div className="flex flex-wrap gap-1 mt-2">{(teacher.levels || [teacher.level]).map(level => (<span key={level} className={`px-2 py-1 text-xs rounded-full ${level === 'Anaokulu' ? 'bg-green-100 text-green-800' : level === 'İlkokul' ? 'bg-blue-100 text-blue-800' : 'bg-ide-accent-100 text-ide-accent-800'}`}>{level}</span>))}</div>
                </div>
      ...
      <Modal isOpen={isModalOpen} onClose={resetForm} title={editingTeacher ? 'Öğretmen Düzenle' : 'Yeni Öğretmen Ekle'} size="lg">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="mt-6 pt-6 border-t">
            <h3 className="text-lg font-medium mb-4 flex items-center"><BookOpen className="w-5 h-5 mr-2 text-ide-primary-600" />Verebileceği Dersler</h3>
