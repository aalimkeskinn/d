...
const WizardStepConstraints: React.FC<WizardStepConstraintsProps> = ({
  ...
  const handleGlobalConstraintChange = (key: string, value: any) => {
    onUpdate({
      constraints: {
        ...(data.constraints || { timeConstraints: [], globalRules: {} }),
        globalRules: {
          ...(data.constraints?.globalRules as any),
          [key]: value
        } as any
      }
    });
    ...
    <div className="space-y-6">
      ...
      <div className="border-b border-gray-200">
        <nav className="-mb-px flex space-x-8">
          {tabs.map((tab) => (
            <button key={tab.id} onClick={() => { setActiveTab(tab.id as any); setSelectedEntity(''); }} className={`py-2 px-1 border-b-2 font-medium text-sm flex items-center ${activeTab === tab.id ? 'border-ide-primary-500 text-ide-primary-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>
